...
MODE_ROLE_SKILLS: dict[str, dict[Role, Skill]] = {
    ...
    "ghost_blank": {
        ...
        Role.VILLAGER: Skill(
            ...
            description="无词版人方藏词",
            instructions=(
                "你与其他人拿到同一个词，而鬼只知道公开的类型和字数。每轮只给一个真实但低辨识度的"
                "间接维度，优先使用主观感受、宽泛比较、弱关联或非专属使用体验；让描述同时兼容该类型"
...
BLANK_GHOST_GAMECRAFT_SKILL = Skill(
    ...
    instructions=(
        ...
        "每轮比较线索的信息量、独立性和跟随痕迹，并保留至少一个反方解释。鬼存活到只剩一名人即"
    ...
    if role_preset == "killer":
        ...
        skill = Skill(
            ...
            instructions=(
                ...
                "词语类型和字数。人每轮使用低辨识度的间接线索描述词牌，绝不能直接说词或拼出唯一答案；"
                "鬼从公开描述中逐步猜词并伪装。鬼存活到只剩一名人即获胜；全部鬼出局后，鬼队先私密"
